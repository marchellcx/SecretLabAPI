...
namespace ObscurisCore.Utilities.Interactions
{
    ...
    public abstract class InteractableObject
    {
        ...
        public bool SpawnInteractable(Vector3 position, Quaternion rotation, Transform? parent = null)
        {
            if (InteractableToy?.Base != null)
                return false;

            InteractableToy = new(position, rotation)
            {
            ...
            InteractableToy.Transform.parent = parent;
        ...
        public bool SpawnInteractable(Transform? parent = null)
        {
            ...
            InteractableToy = new(parent?.position ?? Vector3.zero, parent?.rotation ?? Quaternion.identity)
            {
                Scale = InteractableScale,
                Shape = InteractableShape,
                InteractionDuration = InteractionDuration,
        ...
        /// <param name="parent">The parent of the toy.</param>
        /// <returns>true if the toy was spawned.</returns>
        public bool SpawnInteractable(NetworkIdentity? parent = null)
        {
            if (InteractableToy?.Base != null)
                return false;
        ...
        private void _OnInteracted(PlayerInteractedToyEventArgs args)
        {
            if (args.Interactable?.Base == null
                || InteractableToy?.Base == null
                || InteractableToy.Base != args.Interactable.Base)
                return;
