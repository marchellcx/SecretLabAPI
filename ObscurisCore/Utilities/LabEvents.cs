...
namespace ObscurisCore.Utilities
{
    ...
    public static class LabEvents
    {
        ...
        private static void Initialize()
        {
            ...
            ApiLog.Info("ObscurisCore", $"Patched &3{count}&r event types in &3{watch.ElapsedMilliseconds}&rms.");
        }
        ...
        private static int InitializeAssembly(Assembly assembly)
        {
            ...
            try
            {
                foreach (var type in assembly.GetTypes())
                {
                    ...
                    {
                        ApiLog.Error("ObscurisCore", $"Error while processing event patch for type &3{type}&r:\n{typeEx}");
                    }
                }
            }
            catch (Exception ex)
            {
                ApiLog.Error("ObscurisCore", $"Error while processing event types for assembly &3{assembly}&r:\n{ex}");
            }
