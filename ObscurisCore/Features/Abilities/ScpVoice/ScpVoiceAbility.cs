...
public class ScpVoiceAbility : CustomAbility
{
    ...
    private static string printString = "Mode: %mode%";
    ...
    public override void OnEnabled()
    {
        ...
        if (Player.IsSCP)
        {
            Profile = Player.Voice.AddProfile<ScpVoiceProfile>();
    ...
    public override void OnUsed()
    {
        ...
        var mode = (byte)Mode + 1;

        if (mode >= (byte)ScpVoiceStatus.Mixed)
            mode = 0;
        
        Mode = (ScpVoiceStatus)mode;
    ...
    public override bool Print(StringBuilder builder)
    {
        ...
        {
            builder.Append(printString.Replace("%mode%", Status.ToString()));
            return true;
        }
    ...
    private static void OnEntryCreated(PlayerSettingsEntryCreatedEventArgs args)
    {
        ...
        {
            ApiLog.Debug("Created entry menu is not ScpVoiceMenu");
            return;
        }

        if (args.Player.HasAbility<ScpVoiceAbility>())
        {
            ApiLog.Debug($"Player {args.Player.ToLogString()} already has SCP voice ability");
            return;
        }
        ...
        if (args.Player.AddAbility<ScpVoiceAbility>(out var ability))
        {
            ...
            ApiLog.Debug($"Added SCP voice ability to player {args.Player.ToLogString()}");
        }
        else
        {
