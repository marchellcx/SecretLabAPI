using LabExtended.API;
using SecretLabAPI.Features.Levels.Interfaces;

namespace SecretLabAPI.Features.Levels.Events
{
    /// <summary>
    /// Provides data for the event that occurs when points are added to a player.
    /// </summary>
    public class AddedPointsEventArgs : EventArgs
    {
        /// <summary>
        /// Gets the player receiving the points.
        /// </summary>
        public ExPlayer Player { get; }

        /// <summary>
        /// Gets the reward being given.
        /// </summary>
        public ILevelReward Reward { get; }

        /// <summary>
        /// Gets or sets the amount of points to be given.
        /// </summary>
        public int Amount { get; }

        /// <summary>
        /// Initializes a new instance of the AddedPointsEventArgs class with the specified player, reward, and
        /// points amount.
        /// </summary>
        /// <param name="player">The player who is receiving the points.</param>
        /// <param name="reward">The level reward associated with the points being added.</param>
        /// <param name="amount">The amount of points to be added.</param>
        public AddedPointsEventArgs(ExPlayer player, ILevelReward reward, int amount)
        {
            Player = player;
            Reward = reward;
            Amount = amount;
        }
    }
}