...
{
    [Config("serverMenu", "servers", "List of servers to display in the server menu.")]
    private static List<ServerMenuInfo> servers = new()
    {
        new(),
        new(),
        new()
    ...
    {
        TimingUtils.AfterSeconds(() => { player.AddMenu(new ServerMenuInstance()); }, 2f);
    }
