...
public class EffectActions : CoinAction
{
    ...
    public override void Execute(ExPlayer player)
    {
        ...
        switch (action)
        {
            ...
            case ActionType.EnableRandomMixedEffect:
                EnableEffect(player.Effects.Effects.GetRandomItem(kvp => !IgnoreEffects.Contains(kvp.Key.Name) 
                            && !kvp.Value.IsEnabled
            ...
            case ActionType.DisableRandomMixedEffect:
                player.Effects.Effects.GetRandomItem(kvp => !IgnoreEffects.Contains(kvp.Key.Name) 
                                                            && kvp.Value.IsEnabled 
                                                            && kvp.Value.Classification is StatusEffectBase.EffectClassification.Mixed)
                                      .Value.ServerDisable();
                break;
